...
def main():
    ...
    for ratio_name, ratio_val in ratios.items():
        print(f"\n===================== TARGET RATIO: {ratio_name} ({ratio_val}) =====================")
        for item in images:
            file_name = f"halloween_{ratio_name}_{item['id']}_{item['name']}.png"
            full_prompt = f"{ratio_val} shape. " + item["prompt"]
            ...
            if res and res.get("code") == "200" and res.get("data") and res["data"].get("imageList"):
                ...
                generated_files.append({"aspect": ratio_name, "title": item['name'].replace('_', ' ').title(), "file_name": file_name, "prompt": full_prompt})
            else:
    ...
    preview_md_path = os.path.join(output_dir, "halloween_10_images_preview.md")
    try:
        with open(preview_md_path, "w", encoding="utf-8") as md_file:
            md_file.write("# 🎃 万圣节商店宣传图预览 (16:9 & 1:1 特别版)\n\n")
            md_file.write("本文档由生图脚本自动生成，用于快速预览当次生成的 10 张（横版与方版）插花三消商店图。\n\n")
            
            for ratio in ["landscape", "square"]:
                md_file.write(f"\n## 🖼️ {ratio.capitalize()} Version ({ratios[ratio]})\n\n")
                for item in [x for x in generated_files if x["aspect"] == ratio]:
