...
def main():
    ...
    images = [
        {
            "name": "halloween_img1_sort_magic_flowers.png",
            "prompt": (
        ...
        {
            "name": "halloween_img2_match_merge.png",
            "prompt": (
                "9:16 vertical mobile game store promotional screenshot. "
        ...
        {
            "name": "halloween_img3_spooky_puzzles.png",
            "prompt": (
                "9:16 vertical mobile game store promotional screenshot. "
        ...
        {
            "name": "halloween_img4_use_boosters.png",
            "prompt": (
                "9:16 vertical mobile game store promotional screenshot. "
        ...
        {
            "name": "halloween_img5_magic_collection.png",
            "prompt": (
                "9:16 vertical mobile game store promotional screenshot. "
    ...
    preview_md_path = os.path.join(output_dir, "halloween_images_preview.md")
    try:
        with open(preview_md_path, "w", encoding="utf-8") as md_file:
            md_file.write("# 🎃 万圣节商店宣传图预览 (Halloween Store Images)\n\n")
            md_file.write("本文档由生图脚本自动生成，用于快速预览当次生成的 5 张插花三消商店图。\n\n")
            
            for item in images:
                file_name = item["name"]
                img_title = file_name.replace("halloween_", "").replace(".png", "").replace("_", " ").title()
                md_file.write(f"## {img_title}\n\n")
                md_file.write(f"![{img_title}](file:///{os.path.abspath(os.path.join(output_dir, file_name)).replace(os.sep, '/')})\n\n")
